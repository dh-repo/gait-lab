...
/**
 * Find local extrema in a 1D signal with minimum frame distance constraint.
 */
  ...
  for (let i = 1; i < n - 1; i++) {
    ...
    if (isExtremum) {
      if (indices.length === 0 || i - indices[indices.length - 1] >= minGap) {
        ...
        // Keep the more prominent extremum if within minGap
...
export function detectGaitEventsZeni(
  ...
  // Filter and build left foot events (sequence: IC -> TO -> IC...)
  for (const f of rawLHeelStrikes) {
    const timeSec = frames[f]?.timeMs ? frames[f].timeMs / 1000 : f / effectiveFps;
