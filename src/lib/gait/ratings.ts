...
export function buildStructuredReport(
  ...
  const domains: DomainRating[] = [
    ...
    domain(
      ...
      [
        { label: "Lateral sway", value: m.lateralSway.toFixed(3), hint: m.lateralSway > 0.08 ? "down" : "up" },
        { label: "Vertical bounce", value: m.verticalBounce.toFixed(3) },
        { label: "Step-width var", value: m.stepWidthVariability.toFixed(3) },
      ],
    ),
    domain(
      "symmetry",
      ...
      [
        ...
        { label: "Stride asym", value: `${(m.strideAsymmetry * 100).toFixed(0)}%` },
        { label: "Knee asym", value: `${(m.kneeAsymmetry * 100).toFixed(0)}%` },
  ...
  const metrics: MetricRating[] = [
    ...
    },
    {
      ...
      display: (m.strideAsymmetry * 100).toFixed(0),
      unit: "%",
      favorability: clamp(100 - m.strideAsymmetry * 120, 5, 98),
      band: bandFromBurden(m.strideAsymmetry),
      note: "Proxy from hip travel between steps.",
    },
    {
      id: "sway",
      group: "Stability",
      label: "Lateral sway",
      display: m.lateralSway.toFixed(3),
      unit: "idx",
      favorability: clamp(100 - m.lateralSway * 400, 5, 98),
      band: bandFromBurden(clamp(m.lateralSway * 4, 0, 1)),
      note: "Normalized hip side-to-side residual motion.",
    },
    {
      id: "bounce",
      group: "Stability",
    ...
    },
    {
      ...
      display: m.meanStepWidth.toFixed(3),
      unit: "idx",
      favorability: clamp(100 - Math.abs(m.meanStepWidth - 0.35) * 80, 15, 95),
      band: bandFromScore(clamp(100 - Math.abs(m.meanStepWidth - 0.35) * 80, 15, 95)),
      note: "Frontal/oblique views only; extreme width can be caution.",
    },
    {
      id: "pelvic",
      group: "Stability",
      label: "Pelvic obliquity",
      display: m.pelvicObliquity.toFixed(3),
      unit: "idx",
      favorability: clamp(100 - m.pelvicObliquity * 400, 5, 98),
      band: bandFromBurden(clamp(m.pelvicObliquity * 4, 0, 1)),
      note: "L–R hip height difference proxy (Trendelenburg-ish soft).",
    },
    {
      id: "armL",
      group: "Arms & knees",
    ...
    },
    {
      ...
      group: "Arms & knees",
      label: "Knee flex L",
      display: m.kneeFlexLeft.toFixed(0),
      unit: "°",
      favorability: clamp(m.kneeFlexLeft * 1.2, 10, 95),
      band: bandFromScore(clamp(m.kneeFlexLeft * 1.2, 10, 95)),
      note: "Range of hip–knee–ankle angle (best in side view).",
    },
    {
      id: "kneeR",
      group: "Arms & knees",
      label: "Knee flex R",
      display: m.kneeFlexRight.toFixed(0),
      unit: "°",
      favorability: clamp(m.kneeFlexRight * 1.2, 10, 95),
      band: bandFromScore(clamp(m.kneeFlexRight * 1.2, 10, 95)),
      note: "Range of hip–knee–ankle angle (best in side view).",
    },
    {
      id: "smooth",
