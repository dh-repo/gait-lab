...
// Import analysis functions from compiled/ts source using tsx/node or direct JS test harness
import {
  matchPeople,
  tracksToPeople,
  computeBiometricSignature,
  biometricDistance,
} from '../../src/lib/gait/analysis.js';

console.log("=== EMPIRICAL SINGLE-SUBJECT TRACKING DEDUPLICATION TEST ===");

function mockPersonLandmarks(x, y, height = 0.6, width = 0.2, vis = 0.95) {
...
function runScenario(name, frameGenerator, expectedPeopleCount, isSingleSubject = true) {
  testCount++;
  const tracks = [];
...
runScenario("Scenario 6: Parkinsonian Micro-Steps (Slow Cadence)", () => {
  const frames = [];
  for (let f = 0; f < 60; f++) {
    const x = 0.2 + (f / 59) * 0.15; // very small displacement per frame
...
runScenario("Scenario 7: Pathological Asymmetric Limp", () => {
  ...
  for (let f = 0; f < 60; f++) {
    ...
    const y = 0.5 + (f % 6 < 2 ? 0.03 : 0.0); // asymmetric stance dip
...
runScenario("Scenario 8: Outdoor Follow-Cam (Camera Jitter & Shake)", () => {
  ...
  for (let f = 0; f < 60; f++) {
    ...
    const x = 0.5 + jitterX; // Subject kept roughly centered in follow-cam
    const y = 0.5 + jitterY;
